...
import com.example.backend.common.CurrentUserResolver;
...
public class TaskController {
    ...
    private CurrentUser cu(HttpServletRequest req){
        return (CurrentUser) req.getAttribute(CurrentUserResolver.ATTR);
    }

    @GetMapping public ResponseEntity<List<Task>> list(HttpServletRequest req){
        CurrentUser cu = cu(req);
        if (cu==null) return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        return ResponseEntity.ok(tasks.findByUserIdOrderByCreatedAtDesc(cu.id()));
    }

    @PostMapping public ResponseEntity<Task> create(@RequestBody Task dto, HttpServletRequest req){
        CurrentUser cu = cu(req);
        if (cu==null) return ResponseEntity.badRequest().build();
        ...
        Task t = Task.builder()
                ...
                .dueDate(dto.getDueDate()).createdAt(Instant.now()).updatedAt(Instant.now())
                .user(u).build();
        return ResponseEntity.ok(tasks.save(t));
    }

    @PutMapping("/{id}") public ResponseEntity<Task> update(@PathVariable Long id, @RequestBody Task dto, HttpServletRequest req){
    ...
    @DeleteMapping("/{id}") public ResponseEntity<Void> delete(@PathVariable Long id, HttpServletRequest req){
        CurrentUser cu = cu(req); if (cu==null) return ResponseEntity.badRequest().build();
        Task t = tasks.findById(id).orElseThrow();
        if (!t.getUser().getId().equals(cu.id())) return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
